...
# $Id: check_make_xcode_cfg.sh 157469 2009-04-15 20:02:29Z gouriano $
...
#
# Usage:
#    check_make_win_cfg.sh <action> <solution> <solution_dir> <static|dll> <cfg> [build_dir]
#
#    <action>      - { init | create } 
...
#    <solution_dir>  - solution directory name
#                    (relative path from build directory).
...
# Check list
x_check_dir="$x_build_dir/$x_soldir/${x_solution}.check/$x_cfg"
if test ! -d "$x_check_dir"; then
  x_check_dir="$x_build_dir/$x_soldir/build/${x_solution}.check/$x_cfg"
  if test ! -d "$x_check_dir"; then
...
##   x_cmd="[\$x_wdir]"
   ...
   \$check_exec time -p \`eval echo \$x_run_fix\` > \$x_test_out.\$\$ 2>&1
   result=\$?
